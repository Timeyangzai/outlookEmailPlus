...
def start_device_code() -> Any:
    ...
    device_data, error_info = oauth_tool_service.start_device_code_flow(oauth_config)
    if device_data is None:
        if isinstance(error_info, dict):
            error_code = "OAUTH_CONFIG_INVALID" if error_info.get("config_error") else "OAUTH_MICROSOFT_REQUEST_FAILED"
            return build_error_response(
                error_code,
                error_info.get("message", "启动设备码授权失败"),
                status=400,
                details=error_info.get("guidance"),
            )
        return build_error_response("OAUTH_MICROSOFT_REQUEST_FAILED", str(error_info), status=400)

    session["oauth_device_flow_id"] = device_data["flow_id"]
    log_audit("create", "oauth_device_code", client_id, "Device Code 授权已启动")
    return jsonify({"success": True, "data": device_data})


@login_required
def poll_device_code() -> Any:
    _ensure_oauth_tool_enabled()
    data = request.get_json(silent=True) or {}
    flow_id = (data.get("flow_id") or "").strip()
    if not flow_id:
        return build_error_response("OAUTH_CONFIG_INVALID", "flow_id 不能为空", status=400)

    if session.get("oauth_device_flow_id") != flow_id:
        return build_error_response(
            "OAUTH_MICROSOFT_AUTH_FAILED",
            "设备码授权流程校验失败，请重新开始",
            status=400,
    ...
    token_data, error_info = oauth_tool_service.poll_device_code_flow(flow_id)
    if token_data is not None:
        session.pop("oauth_device_flow_id", None)
        log_audit("create", "oauth_token", token_data.get("client_id", ""), "Device Code Token 获取成功")
        return jsonify({"success": True, "status": "complete", "data": token_data})

    if isinstance(error_info, dict) and error_info.get("pending"):
        return jsonify(
            {
                "success": True,
                "status": "pending",
                "data": {
                    "message": error_info.get("message", "等待用户完成授权"),
                    "interval": error_info.get("interval", 5),
                    "error": error_info.get("error", "authorization_pending"),
                },
            }
        )
    ...
    if isinstance(error_info, dict):
        return build_error_response(
            "OAUTH_MICROSOFT_REQUEST_FAILED",
            error_info.get("message", "设备码授权失败"),
            status=400,
            details=error_info.get("guidance"),
        )
    return build_error_response("OAUTH_MICROSOFT_REQUEST_FAILED", str(error_info), status=400)
